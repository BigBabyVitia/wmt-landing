...
import { forwardRef, useImperativeHandle, useEffect, useRef, useMemo, type ReactNode } from 'react';
...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: ExtendMaterialConfig) {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    uniforms[key] =
      u !== null && typeof u === 'object' && 'value' in u ? (u as THREE.IUniform) : { value: u };
  });
...
export interface BeamsProps {
  ...
  scale?: number;
  rotation?: number;
  /** Scene background colour. Defaults to black; pass a light colour for a light theme. */
  background?: string;
  /** Albedo of the beam geometry. Defaults to black (warm-lit ribbons on a dark bg);
   *  pass a warm/light colour for a light theme so the beams read on a light background. */
  beamColor?: string;
}
