...
const overviewPrograms = [
  ...
  },
  {
    ...
    desc: "Малая группа из одной компании глубоко разбирает n8n, Claude и OpenClaw, собирает агентов под реальные процессы",
...
const programs = [
  {
    ...
    when: "Когда компании нужно собрать сильную команду, которая сможет работать с ИИ руками, создавать рабочие решения на своих задачах и становиться внутренней опорой ИИ-трансформации.",
    whenBullets: ["Компании нужно собрать сильную команду для работы с ИИ руками", "Которая будет создавать рабочие решения на своих задачах", "И становиться внутренней опорой ИИ-трансформации"],
    result: "Участники настраивают своё ИИ-рабочее место, собирают первого агента, решают бизнес-кейс компании и выходят с личным планом на 30 дней. Компания получает практиков, которая уверенно развивает ИИ-подход внутри бизнеса",
    ...
    desc: "Команда собирает первых агентов на своих задачах и становится ядром ИИ-трансформации. Участники берут реальные задачи компании, собирают агентов, проверяют гипотезы и уходят с рабочими артефактами. После того внутри компании появляются свои люди-носители навыка — те, кто может продолжить тему без внешней поддержки.",
    params: [
      { label: "Длительность", value: "1 день / 8 ч", icon: Clock },
      { label: "Практика", value: "80% руками", icon: Zap },
      { label: "Группа", value: "до 20 человек", icon: Users },
    ],
    results: [
      { title: "Ядро практиков", desc: "до 20 человек с реальным опытом работы с ИИ.", icon: CheckCircle2 },
  ...
  {
    id: "ai-wave",
    badge: "Сквозная задача",
    title: "ИИ-Марафон",
    tagline: "Не курс — рабочий разговор. Сквозная задача через 9 блоков",
    when: "Когда команде нужно за месяц пройти через все ключевые инструменты — от промптинга и Claude Projects до RAG-агента в n8n и Claude Code — со сквозной задачей, которая проходит через все блоки и к финалу превращается в рабочий инструмент.",
    whenBullets: ["Команде нужно пройти 9 блоков со сквозной задачей", "Освоить промптинг, Claude Projects, n8n, RAG, Claude Code", "Получить рабочий инструмент к финалу, а не папку конспектов"],
    result: "К финалу — рабочий ИИ-агент с RAG, инструментами и памятью под вашу задачу. Сквозная задача проходит через все 9 блоков: каждый шаг достраивает результат, который команда забирает в работу.",
    resultBullets: ["Рабочий ИИ-агент с RAG, инструментами и памятью под вашу задачу", "Сквозная задача доведена до результата через все 9 блоков", "Команда уверенно владеет промптингом, n8n, Claude Projects и Claude Code"],
    desc: "Не курс — рабочий разговор. 9 блоков по 1,5 часа, онлайн, группа до 12 человек. Сквозная задача проходит через все блоки: от промптинга и работы с файлами до RAG-базы по вашим документам, агента в n8n и Claude Code. К финалу — рабочий инструмент, не папка конспектов.",
    params: [
      { label: "Срок", value: "9 модулей × 1,5 ч", icon: Clock },
      { label: "Занятия", value: "80% практики", icon: Zap },
      { label: "Масштаб", value: "до 12 человек", icon: Users },
    ],
    results: [
      { title: "Рабочий агент", desc: "RAG, инструменты, память — собрано под вашу задачу.", icon: CheckCircle2 },
      { title: "Сквозная задача", desc: "Один кейс проходит через все 9 блоков до результата.", icon: CheckCircle2 },
      { title: "Стек инструментов", desc: "Промптинг, Claude Projects, n8n, Claude Code — освоены руками.", icon: CheckCircle2 },
    ],
  },
  {
    id: "builder-day",
    ...
    result: "Команда уходит с рабочим ИИ-агентом, настроенным workflow, подключённой логикой и алгоритмом повторной сборки за 30–60 минут под новые задачи.",
    resultBullets: ["Команда уходит с рабочим ИИ-агентом и настроенным workflow", "Идет работа с подключённой логикой", "Команда получает алгоритм повторной сборки за 30–60 минут"],
    ...
    params: [
      { label: "Длительность", value: "1 день / 6 ч", icon: Clock },
      { label: "Практика", value: "80–85% руками", icon: Zap },
      { label: "Группа", value: "до 15 человек", icon: Users },
    ],
    results: [
  ...
  },
  {
    ...
    desc: "Малая группа до 6 человек из одной компании. 7 занятий, ~12 часов практики. Последовательное погружение: обзорное (3 платформы на одной задаче) → n8n базовое → ИИ-агенты в n8n → Claude (Cowork, Projects, промптинг) → Claude Code → OpenClaw + RAG → боевая сборка multi-agent с safety. Идеально для команды, которая готова стать внутренним архитектором ИИ-решений.",
    params: [
      { label: "Срок", value: "7 модулей / ~12 ч", icon: Clock },
  ...
  },
  {
    ...
    when: "Когда ИИ-трансформация команды требует собственной программы: с учётом целей бизнеса, состава участников, уровня зрелости, процессов и того результата, который компания хочет получить на уровне практики и масштабирования.",
    whenBullets: ["ИИ-трансформация команды требует собственной программы", "С учётом целей бизнеса, состава участников и уровня зрелости процессов", "С учётом того результата, который компания хочет получить"],
    result: "Компания получает индивидуально собранную программу под свой контекст, а руководство и команда — формат, который точно соответствует их задачам и даёт следующий сильный шаг в ИИ-трансформации.",
    resultBullets: ["Компания получает индивидуально собранную программу под свой контекст", "Формат точно соответствует задачам руководства и команды", "Решение даёт следующий сильный шаг в ИИ-трансформации"],
    desc: "Собираем программу ИИ-трансформации под ваши задачи, команду, процессы и нужный масштаб внедрения. Мы проводим предварительный аудит, интервьюируем ключевых участников и собираем формат, который даст максимальный эффект именно вашему подразделению.",
    params: [
  ...
  return (
    <>
      <V2Hero 
        ...
        title={<><span className="text-brand">ИИ-трансформация</span> для команд: от практики к масштабированию</>}
    ...
    <section ref={ref} className="py-16 md:py-32 px-4 sm:px-6 md:px-12 bg-white dark:bg-black border-t border-gray-100 dark:border-white/[0.06] transition-colors duration-300 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className={`mb-10 md:mb-16 transition-all duration-1000 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <p className="text-base md:text-xl text-gray-500 dark:text-gray-400 max-w-5xl font-medium leading-relaxed">От сборки первых агентов до масштабирования ИИ-привычек на процессы подразделений.</p>
        </div>
