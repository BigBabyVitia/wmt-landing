  ...
  return (
    <section id="home" className="relative w-full h-screen overflow-hidden bg-black flex flex-col pt-24 pb-40">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-6 mt-12">
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-8 animate-fade-rise-delay-2">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-12 animate-fade-rise-delay-2">
          <a href="#directions" className="liquid-glass rounded-full px-12 py-4 text-base font-medium text-white hover:scale-[1.03] transition-transform cursor-pointer inline-block">
            Узнать, что подойдёт
          </a>
          <Link to="/all-formats" className="liquid-glass rounded-full px-12 py-4 text-base font-medium text-white hover:scale-[1.03] transition-transform cursor-pointer inline-block">
            Сравнить программы
          </Link>
        </div>
      </div>
...
type ContrastMode = typeof contrastOptions[number]["id"]

function HeroWebGL() {
  const [mode, setMode] = useState<ContrastMode>("combo")

  const overlayClass =
    mode === "overlay" ? "bg-black/45"
    : mode === "combo" ? "bg-black/25"
    : ""

  const gradientClass =
    mode === "gradient" ? "bg-gradient-to-t from-black/60 via-transparent to-black/30"
    : ""

  const subShadow = mode === "combo" ? "0 1px 12px rgba(0,0,0,0.4)" : undefined
  ...
  return (
    <section id="home" className="relative w-full h-screen overflow-hidden bg-black flex flex-col pt-24 pb-40">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-6 mt-12">
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-8 animate-fade-rise-delay-2">
          {["50+ проектов 2024–2026", "80–85% практики руками", "Рабочие агенты на выходе"].map((t) => (
            <span key={t} className="liquid-glass text-white px-4 py-2 rounded-full text-sm font-medium" style={subShadow ? { textShadow: subShadow } : undefined}>{t}</span>
          ))}
        </div>
        <div className="flex flex-col sm:flex-row gap-4 mt-12 animate-fade-rise-delay-2">
          <a href="#directions" className="liquid-glass rounded-full px-12 py-4 text-base font-medium text-white hover:scale-[1.03] transition-transform cursor-pointer inline-block">
      ...
      <div className="p-8 md:p-14 relative z-10 flex flex-col gap-10">
        ...
        {items.length > 0 && (
          <div className="w-full">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-5 text-center xl:text-left">Программы направления</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map((item: string, idx: number) => {
                const parts = item.split(" — ");
                return (
                  <div key={idx} className="flex flex-col gap-1.5 bg-gray-50/80 border border-gray-100 rounded-2xl p-5 hover:bg-gray-100/80 transition-colors content-start">
...
function CombinedLevelsViewer() {
  ...
  return (
    <div className="flex flex-col items-center pb-24 border-t border-gray-100 bg-gray-50/50 pt-24 min-h-screen">
      <div className="text-center mb-8">
        <span className="bg-brand/10 text-brand px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">DEV MODE</span>
        <h2 className="text-2xl font-semibold mt-4 text-gray-900">Выберите вариант позиционирования</h2>
      </div>

      <div className="flex flex-wrap justify-center gap-2 bg-gray-200/50 p-1.5 rounded-full mb-16 shadow-inner mx-auto max-w-max">
        <button onClick={() => setVariant(1)} className={`px-6 py-2.5 rounded-full text-sm font-medium transition-all ${variant === 1 ? "bg-white text-gray-900 shadow" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100/50"}`}>
          1. Слияние (Всё в карточках)
        </button>
        <button onClick={() => setVariant(2)} className={`px-6 py-2.5 rounded-full text-sm font-medium transition-all ${variant === 2 ? "bg-white text-gray-900 shadow" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100/50"}`}>
          2. Инфографика + Каталог
        </button>
        <button onClick={() => setVariant(3)} className={`px-6 py-2.5 rounded-full text-sm font-medium transition-all ${variant === 3 ? "bg-white text-gray-900 shadow" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100/50"}`}>
          3. Интерактивные Табы
        </button>
      </div>

      <div className="w-full">
        {variant === 1 && <Variant1Merge />}
        {variant === 2 && <Variant2Infographic />}
        {variant === 3 && <Variant3Tabs />}
      </div>
    </div>
  )
}

function Variant1Merge() {
  const mergedData = [
    {
      ...teaserData[0],
  ...
  return (
    <div className="px-6 md:px-12 w-full max-w-7xl mx-auto flex flex-col gap-10">
      {mergedData.map((t, i) => <TeaserCard key={t.id} {...t} index={i} />)}
    </div>
  )
}

function Variant2Infographic() {
  return (
    <div className="px-6 md:px-12 w-full flex flex-col items-center">
      <div className="max-w-4xl w-full mx-auto mb-20 bg-white p-8 md:p-12 rounded-[2.5rem] border border-gray-100 shadow-sm relative overflow-hidden">
        <h2 className="text-2xl font-semibold tracking-tight text-center text-gray-900 mb-10">
          Уровни готовности к ИИ
        </h2>
        
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 relative z-10">
          <div className="absolute top-1/2 left-[10%] w-[80%] h-0.5 bg-gradient-to-r from-gray-100 via-brand/30 to-gray-100 hidden md:block -translate-y-1/2" />
          
          <div className="relative z-10 flex flex-col items-center min-w-[180px] text-center bg-white p-2">
            <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 mb-4 text-xl font-bold border-4 border-white shadow-sm">1</div>
            <strong className="text-lg text-gray-900 mb-1">Руководство</strong>
            <span className="text-sm text-gray-500">Принятие решения</span>
          </div>

          <ArrowRight className="text-gray-300 md:hidden w-8 h-8 rotate-90 my-2" />

          <div className="relative z-10 flex flex-col items-center min-w-[180px] text-center bg-white p-2">
            <div className="w-16 h-16 bg-brand/10 rounded-full flex items-center justify-center text-brand mb-4 text-xl font-bold border-4 border-white shadow-sm">2</div>
            <strong className="text-lg text-gray-900 mb-1">Команды</strong>
            <span className="text-sm text-brand font-medium">Масштаб практики</span>
          </div>

          <ArrowRight className="text-gray-300 md:hidden w-8 h-8 rotate-90 my-2" />

          <div className="relative z-10 flex flex-col items-center min-w-[180px] text-center bg-white p-2">
            <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 mb-4 text-xl font-bold border-4 border-white shadow-sm">3</div>
            <strong className="text-lg text-gray-900 mb-1">Лично</strong>
            <span className="text-sm text-gray-500">Углубление</span>
          </div>
        </div>
      </div>
...
function Variant3Tabs() {
  const [activeTab, setActiveTab] = useState(0)
  const steps = [
    { title: "Управленческий ход", sub: "Стратегия и воля" },
    { title: "Команды", sub: "Масштаб практики" },
    { title: "Лично", sub: "Погружение руками" }
  ]
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-3 sticky top-32">
          {steps.map((step, i) => (
            <button 
              ...
              className={`text-left p-6 md:p-8 rounded-[2rem] border transition-all duration-300 flex flex-col gap-2 relative overflow-hidden ${activeTab === i ? "bg-white border-brand/30 shadow-xl shadow-brand/5 scale-[1.02]" : "bg-white/50 border-gray-100 hover:bg-white hover:shadow-md"}`}
            >
              <div className={`absolute top-0 right-0 w-24 h-24 bg-brand rounded-bl-full transition-opacity duration-300 -z-10 blur-2xl ${activeTab === i ? "opacity-10" : "opacity-0"}`} />
              
              <div className="flex items-center gap-3 mb-2">
                 <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold transition-colors ${activeTab === i ? "bg-brand text-white" : "bg-gray-100 text-gray-400"}`}>
                   {i + 1}
                 </div>
                 <span className={`text-xs font-semibold uppercase tracking-wider ${activeTab === i ? "text-brand" : "text-gray-400"}`}>
                   Уровень
                 </span>
              </div>
              <strong className="text-2xl text-gray-900">{step.title}</strong>
              <span className="text-gray-500 text-base">{step.sub}</span>
            </button>
          ))}
        </div>

        {/* Right column: Active Card */}
        <div className="w-full lg:w-2/3 lg:min-h-[600px] flex items-stretch">
          <AnimatePresence mode="wait">
             <motion.div
               ...
               className="w-full"
             >
               <TeaserCard {...teaserData[activeTab]} index={0} />
             </motion.div>
          </AnimatePresence>
        </div>
      </div>
